...
define(`drawAsBox', `ctx.fillRect( @x - eval(upcase($1)_WIDTH / 2), @y - eval(upcase($1)_HEIGHT / 2), upcase($1)_WIDTH, upcase($1)_HEIGHT )')dnl
...
gameloop = ->
  ...
  game.powerups.healthups = game.powerups.healthups.concat( new HealthUp( enemy.x, enemy.y ) for enemy in game.owners.enemies.units when enemy.health <= 0 and Math.random() < HEALTHUP_RAND )
